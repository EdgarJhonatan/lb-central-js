...
module.exports = function (listarSucursal) {
  listarSucursal.listarGet = function (articulo, tipoAccion, cb) {
    const main = async () => {
      try {
        console.log(articulo, tipoAccion);
        var bodyJson;
        console.log(`========== Consultamos a la Base de datos  ==========`);
        const query = `select * from public.ws_listar_articulo ('${articulo}', '${tipoAccion}');`;
        console.log(query);
